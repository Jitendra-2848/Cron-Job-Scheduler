...
const seed = async () => {
    ...
    try {
        ...
        for (const job of jobs) {
            const [userId,name,url,method,cronExpression,payload,retries,status] = job;

            let nextRunAt = null;

            if (status === "active") {
                nextRunAt = CronExpressionParser
                    .parse(cronExpression as string,{
            ...
            await client.query(`
                INSERT INTO jobs
                (user_id,name,url,method,cron_expression,payload,retries,status,next_run_at)
